...
<body>
    ...
    <a href="#" id="botonAgregar" class="floating-button" onclick="agregarNivel()"><i class="fas fa-plus"></i></a>
    ...
    <script>
        $(document).ready(function () {
            ...
            function cargarNiveles() {
                ...
                $.ajax({
                    url: '../php/service/ServiceAdmin.php?opcion=gtn', // Reemplaza 'ruta_al_archivo_php.php' por la ruta correcta a tu archivo PHP para obtener los niveles
                    dataType: 'json',
                    type: 'get',
                    success: function (data) {
                        ...
                        $.each(data, function (index, nivel) {
                            ...
                            // Aquí creamos la estructura HTML para mostrar los niveles
                            html +=
                                '<div class="card fondoCard">' +
                                '<div class="card-body">' +
                                '<h1 class="card-title">' + nivel.nivelNombre + '</h1>' +
                                '</div>' +
                                '<div class="card-footer">' +
                                '<p>Adicional: ' + nivel.adicional + '</p>' +
                                '<p>Numero de Desafios: ' + nivel.numeroDesafios + '</p>' +
                                '<button class="btn btn-danger float-end" onclick="eliminarNivel(' + nivel.nivelId + ')"><i class="fa fa-trash"></i></button>' +
                                '<button class="btn btn-purple float-end" onclick="editarNivel(' + nivel.nivelId + ', \'' + nivel.nivelNombre + '\', \'' + nivel.nivelDesc + '\', \'' + nivel.adicional + '\', \'' + nivel.numeroDesafios + '\'' + ')"><i class="fa fa-edit"></i></button>' +
                                '</div>' +
                                '</div>';

                            listItem.html(html);
                            html = '';
                            // Agregar el nivel al contenedor
                            listView.append(listItem);
                        });
                    },
                    error: function () {
                        console.log('Error al obtener los niveles.');
                    }
                });
            }

            // Llamada inicial para cargar todos los niveles al cargar la página
            cargarNiveles();
        });
        ...
        // Función para eliminar un nivel mediante AJAX
        function eliminarNivel(nivelId) {
            $.ajax({
                url: '../php/service/ServiceAdmin.php', // Reemplaza 'ruta_al_archivo_php.php' por la ruta correcta a tu archivo PHP para eliminar el nivel
                type: 'get',
                data: {
                    opcion: 'actualizaNivel',
                    id: nivelId,
                    estatus: 0
                },
                dataType: 'json',
                success: function (data) {
                    ...
                    var mensaje = data.success ? 'Nivel eliminado exitosamente.' : 'Ocurrió un error al eliminar el nivel.';
                    var alertClass = data.success ? 'alert-success' : 'alert-danger';
                    showAlert(mensaje, alertClass);
                    scrollToTop();
                    // Eliminar el nivel de la lista en el front-end si se eliminó exitosamente en el backend
                    if (data.success) {
                        $('#listView').empty();
                        cargarNiveles();
                    }
                ...
                error: function () {
                    showAlert('Error al eliminar el nivel.', 'alert-danger');
                    scrollToTop();
                }
            });
        }
